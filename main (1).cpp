...
// Aima Qutbuddin 
// Date: 12/01/2023
// Professor Theys, Fall 2023, 11 am Lecture
// Enviorment: Replit and Zybooks 
...
// In order to create aand correclty execute these funcitons, I utilized a class and implemented a linked lsit to store the moves of each player.  
// Pass by reference allowed me to manipulate the baords and pointers assoaciated with each node. 
...
class gameBoard{ // creating a class, 
    ...
    char board[6][7]; // 2D arrray is the data type stored to node 
    gameBoard* next; // pointer called next, used to point to the next node in list 
    ...
    // class method
    void displayBoard(){ // display function for the output after each update to the game baord 
...
gameBoard* makePlay(char column, gameBoard*& head, char &turn){ // parameters for the function, notice pas by reference to direclty manipulate the board
    // changing the char to an integer to utlize the column value as an integer
    ...
    // creating a linnk between the new board to the previous one
    // pushing in nodes through the head of the list, newest node added becomes the head of the list 
    updatedBoard->next = head;
    head = updatedBoard; // head is reassigned to the most up to date board

    return head;// returning the most updated board as the head board
}
// function that outputs all the moves made by the users until the pint of a user input of 'P' or 'p'
void PrintMovesMade(gameBoard* head){ //pointer, to manitpulate the board assocaited with the pointer 
    if(head == nullptr){ // conditinoal chekcing if the head pointer is null
        return;
    }
    PrintMovesMade(head->next); // recursive all to output the boards in order of no move made, to most updated board
    head->displayBoard(); // actually displaying the head board assoicaied with the head node. 
    cout<<endl;
}
...
gameBoard* undoMove(gameBoard*& head){ 
    if(head->next == nullptr){ // condition checking if the next pointer is null, 
        ...
        emptyBoard.displayBoard(); // displaying and emppty board, when the nex tpinter goes to null
        return nullptr; // no more undo moves to be made 
    }

    gameBoard* tempBoard = head; // create temporary baord and store the head 
    head = head->next; // assigning the head to the board previous to it, wiht one les move than the most updated board 
    delete tempBoard; // rmeoving the temprary board holding the previously most updated baord 
...
bool columnFull(int column, gameBoard* head){
    for (int row = 5 ; row >= 0 ; row--){ //looping thorugh all the rows of that column 
        if (head->board[row][column] == '-'){ // when dash detected the colunm  is not full
...
// making sure that 4 spaces within a row are allt he same piece either X or O
bool checkHorizontalWin(gameBoard* head, char turn){
    for(int row = 5; row >= 0; row--){ //decrementaing to start checking on the upper side of the baord
...
bool checkVerticalWin(gameBoard* head, char turn){
    for(int row = 5; row >= 3; row--){ //decrementaing to start checking on the upper side of the baord
        for(int column = 0; column < 7; column++){ 
...
bool checkDiagonalWin1(gameBoard* head, char turn){
    for (int row = 5; row >= 3; row--) { //decrementaing to start checking on the upper side of the board
...
bool boardFull(gameBoard* head){
    for(int row = 0; row < 6; row++){ //looping throught the 2D array
        for(int column = 0; column< 7; column++){ 
            if(head->board[row][column] == '-'){ // when dash detected the baord is not full
                return false; //false, baord is not full 
            }
        }
    }
...
int main(){
    ...
    // intializing variables, specifically the columnPiece, the gameBoard, and 
    // the turn of the current player --> alway set to 'X' first
    char columnPiece; 
    char playerTurn = 'X'; 
    ...
    while(true){ //gameplay loop 
        // prompting for input with cout statement and recieving input with cin statement
        cout << "It is " << playerTurn << "'s turn." << endl;
        cout << "Enter a column to place your piece.";
        cin >> columnPiece;
        //cout << endl; 

        if (columnPiece == 'Q' || columnPiece == 'q'){ // in the case where user would like to quit the game
            break; // break out of loop, no more gameplay 
        }
            ...
            if (head!= nullptr){  // when the head not null, switch the user becasue undo counts as a play 
                playerTurn = (playerTurn == 'X') ? 'O' : 'X';
            }
            continue;// continue gameplay 
        }

        if(isdigit(columnPiece)){ // chekcing if the user entered char is a digit or not 
            ...
            if (columnInput >= 0 && columnInput < 7){ 
                // making sure there is room within their chosen column for move to be made 
                if(head!=nullptr){
                    if(columnFull(columnInput, head)){ 
                        cout << "column chosen is already full" << endl;
                        continue; // break out of current iteration and start the next 
                    }
                }
                //calling function to make move and update the board with the board that has the move made 
        ...
        else if (boardFull(initialBoard) == true){ // when condition for a draw between palyers is detected
